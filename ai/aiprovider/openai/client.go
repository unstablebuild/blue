...
type Config struct {
	...
	// parameter to monitor changes in the aiprovider.
	// In beta, currently.
	// Seed int
...
func (a client) CreateChatCompletion(
	ctx context.Context, request aiprovider.ChatCompletionRequest,
) (iterator.Iterator[aiprovider.ChatCompletionResponse], error) {
	...
	if count > max {
		return nil, &aiprovider.ErrContextWindowExceeded{Count: count, Max: max}
	}
	...
	req := openai.ChatCompletionRequest{
		...
		// which greatly simplifies the aiprovider.Service interface.
		// N:                1,
	}
...
func (a client) ExceedsContextWindow(messages []aiprovider.ChatCompletionMessage) (bool, error) {
	return a.countTokens(messages) > a.modelContextWindow[a.config.Model], nil
}

// OpenAI Cookbook: https://github.com/openai/openai-cookbook/blob/main/examples/How_to_count_tokens_with_tiktoken.ipynb
func (a client) countTokens(messages []aiprovider.ChatCompletionMessage) (ret int) {
...
func (s *completionStreamIterator) Next(ctx context.Context) (
	ret aiprovider.ChatCompletionResponse, ok bool,
) {
	...
	s.client.log(log.TraceLevel, "received aiprovider response: %+v", choice)

	if s.role == "" {
		switch choice.Delta.Role {
		case openai.ChatMessageRoleSystem:
			s.role = aiprovider.RoleSystem
		case openai.ChatMessageRoleUser:
			s.role = aiprovider.RoleUser
		case openai.ChatMessageRoleAssistant:
			s.role = aiprovider.RoleAssistant
		case openai.ChatMessageRoleFunction:
			s.role = "function"
		case openai.ChatMessageRoleTool:
			s.role = aiprovider.RoleTool
		default:
			s.err = fmt.Errorf("role %q is unrecognized by aiprovider API", choice.Delta.Role)
		}
	}
	ok = true

	// only set Metadata if ToolCalls is not 0
	// this way clients agnostic to openai's aiprovider
	...
	ret = aiprovider.ChatCompletionResponse{
		ID:      resp.ID,
		Created: time.Unix(resp.Created, 0),
		Message: aiprovider.ChatCompletionMessage{
			Role:     s.role,
		...
		FinishReason: aiprovider.FinishReason(choice.FinishReason),
	}
