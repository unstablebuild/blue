...
import (
	...
	"github.com/unstablebuild/blue/ai/aiprovider"
)

// NOTE: this serves as an example of how to use the aiprovider API backed by openai.
...
func TestCreateChatCompletion(t *testing.T) {
	...
	t.Run("sends a chat completion request, with no context", func(t *testing.T) {
		...
		req := aiprovider.ChatCompletionRequest{Messages: []aiprovider.ChatCompletionMessage{
			{Role: aiprovider.RoleUser, Content: "hello sir!"},
		}}
		it, err := client.CreateChatCompletion(ctx, req)
		require.NoError(t, err)

		var finishReason aiprovider.FinishReason
		...
		for i := 0; ; i++ {
			...
			assert.Equal(t, aiprovider.RoleAssistant, resp.Message.Role)
		...
		assert.Equal(t, aiprovider.FinishReasonStop, finishReason)
	})
	...
	t.Run("sends a chat completion request, with some context, with default configuration", func(t *testing.T) {
		...
		req := aiprovider.ChatCompletionRequest{Messages: []aiprovider.ChatCompletionMessage{
			...
			{Role: aiprovider.RoleUser, Content: "what is your purpouse?"},
		}}
		it, err := client.CreateChatCompletion(ctx, req)
		require.NoError(t, err)
	...
	t.Run("uses tools provided", func(t *testing.T) {
		...
		ctx := context.Background()
		req := aiprovider.ChatCompletionRequest{Messages: []aiprovider.ChatCompletionMessage{
			{Role: aiprovider.RoleUser, Content: "what's the weather like in San Francisco right now?"},
		}}
		it, err := client.CreateChatCompletion(ctx, req)
		require.NoError(t, err)

		var last aiprovider.ChatCompletionResponse
		for i := 0; ; i++ {
			var ok bool
			resp, ok := it.Next(ctx)
			if !ok {
		...
		assert.Equal(t, aiprovider.FinishReasonToolCall, last.FinishReason)
...
func TestContextWindows(t *testing.T) {
	...
	t.Run("CreateCompletionRequest errors with ErrContextWindowExceeded", func(t *testing.T) {
		...
		// sut
		req := aiprovider.ChatCompletionRequest{Messages: msgs}
		_, err := client.CreateChatCompletion(ctx, req)
		require.True(t, errors.Is(err, &aiprovider.ErrContextWindowExceeded{}), err.Error())
	})
...
func TestCountTokens(t *testing.T) {
	for model := range AvailableModels() {
		t.Run(model, func(t *testing.T) {
			...
			text := "¡Hola mundo!"
			assert.Equal(t, 10, client.countTokens([]aiprovider.ChatCompletionMessage{{Content: text}}))
		})
	}
}

func makeMessageTokens(c client, greaterThan int) []aiprovider.ChatCompletionMessage {
	var msgs []aiprovider.ChatCompletionMessage
	for i := 0; c.countTokens(msgs) < greaterThan; i++ {
		msgs = append(msgs, aiprovider.ChatCompletionMessage{
			Content: strconv.Itoa(i), Role: aiprovider.RoleUser,
		})
	}
	return msgs
}
